...
public class CropOverlayView extends View {
    ...
    /**
     *  The Paint used to draw the circle of the size
     */
     ...
     */
    private com.oguogu.cropper.CropImageView.Guidelines mGuidelines;

    /**
     * The shape of the cropping area - rectangle/circular.
     */
    private com.oguogu.cropper.CropImageView.CropShape mCropShape;
    ...
    public void resetCropOverlayView() {
        if (initializedCropWindow) {
            setCropWindowRect(com.oguogu.cropper.BitmapUtils.EMPTY_RECT_F);
            initCropWindow();
            invalidate();
        }
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public com.oguogu.cropper.CropImageView.CropShape getCropShape() {
        return mCropShape;
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public void setCropShape(com.oguogu.cropper.CropImageView.CropShape cropShape) {
        if (mCropShape != cropShape) {
            mCropShape = cropShape;
            if (Build.VERSION.SDK_INT >= 11 && Build.VERSION.SDK_INT <= 17) {
                if (mCropShape == com.oguogu.cropper.CropImageView.CropShape.OVAL) {
     ...
     */
    public com.oguogu.cropper.CropImageView.Guidelines getGuidelines() {
        return mGuidelines;
    }
     ...
     */
    public void setGuidelines(com.oguogu.cropper.CropImageView.Guidelines guidelines) {
    ...
    public void setInitialCropWindowRect(Rect rect) {
        mInitialCropWindowRect.set(rect != null ? rect : com.oguogu.cropper.BitmapUtils.EMPTY_RECT);
        if (initializedCropWindow) {
            initCropWindow();
            invalidate();
     ...
     */
    public void setInitialAttributeValues(com.oguogu.cropper.CropImageOptions options) {
        ...
        mCirclePaint = getNewCirclePaint(options.circleSize, options.circleColor);
    }
    ...
    private void initCropWindow() {
        ...
        float leftLimit = Math.max(com.oguogu.cropper.BitmapUtils.getRectLeft(mBoundsPoints), 0);
        float topLimit = Math.max(com.oguogu.cropper.BitmapUtils.getRectTop(mBoundsPoints), 0);
        float rightLimit = Math.min(com.oguogu.cropper.BitmapUtils.getRectRight(mBoundsPoints), getWidth());
        float bottomLimit = Math.min(com.oguogu.cropper.BitmapUtils.getRectBottom(mBoundsPoints), getHeight());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (mGuidelines == com.oguogu.cropper.CropImageView.Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (mGuidelines == com.oguogu.cropper.CropImageView.Guidelines.ON_TOUCH && mMoveHandler != null) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        float left = Math.max(com.oguogu.cropper.BitmapUtils.getRectLeft(mBoundsPoints), 0);
        float top = Math.max(com.oguogu.cropper.BitmapUtils.getRectTop(mBoundsPoints), 0);
        float right = Math.min(com.oguogu.cropper.BitmapUtils.getRectRight(mBoundsPoints), getWidth());
        float bottom = Math.min(com.oguogu.cropper.BitmapUtils.getRectBottom(mBoundsPoints), getHeight());

        if (mCropShape == com.oguogu.cropper.CropImageView.CropShape.RECTANGLE) {
            ...
            mPath.reset();
            if (Build.VERSION.SDK_INT >= 11 && Build.VERSION.SDK_INT <= 17 && mCropShape == com.oguogu.cropper.CropImageView.CropShape.OVAL) {
    ...
    private void drawCornerCircle(Canvas canvas) {
        if(mCirclePaint != null) {
            ...
            float circleOffset = mCirclePaint.getStrokeWidth() / 2;

            canvas.drawCircle(rect.left - circleOffset, rect.top - circleOffset, mCirclePaint.getStrokeWidth(), mCirclePaint);
            canvas.drawCircle(rect.right + circleOffset, rect.top - circleOffset, mCirclePaint.getStrokeWidth(), mCirclePaint);
            canvas.drawCircle(rect.left - circleOffset, rect.bottom + circleOffset, mCirclePaint.getStrokeWidth(), mCirclePaint);
            canvas.drawCircle(rect.right + circleOffset, rect.bottom + circleOffset, mCirclePaint.getStrokeWidth(), mCirclePaint);
        }
    }
    ...
    private static Paint getNewCirclePaint(float thickness, int color) {
        if (thickness > 0) {
            Paint borderPaint = new Paint();
            borderPaint.setColor(color);
            borderPaint.setStrokeWidth(thickness);
    ...
    private boolean calculateBounds(RectF rect) {
        ...
        float left = com.oguogu.cropper.BitmapUtils.getRectLeft(mBoundsPoints);
        float top = com.oguogu.cropper.BitmapUtils.getRectTop(mBoundsPoints);
        float right = com.oguogu.cropper.BitmapUtils.getRectRight(mBoundsPoints);
        float bottom = com.oguogu.cropper.BitmapUtils.getRectBottom(mBoundsPoints);
